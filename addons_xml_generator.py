...
class Generator:
    ...
    def _generate_addons_file(self):
        # addon list
        addons = os.listdir(self.ROOT_PATH)
        # final addons text
        ...
        for addon in addons:
            try:
                ...
                if (not os.path.isdir(os.path.join(self.ROOT_PATH, addon)) or addon == "data" or addon == ".idea" or addon == ".svn" or addon == ".git" or addon == "zips"): continue
                # create path
                _path = os.path.join(self.ROOT_PATH, addon, "addon.xml")
...
if (__name__ == "__main__"):
    ...
    try:
        ...
        filesinrootdir = os.listdir(ROOT_DIR)
        ...
        for x in filesinrootdir:
            if re.search("^(plugin|script|service|skin|repository)" , x) and not re.search('.zip', x):
                ...
                if "addon.xml" in filesinfoldertozip:
                    tree = ET.parse(os.path.join(ROOT_DIR, x, "addon.xml"))
                    root = tree.getroot()
                ...
                for y in filesinfoldertozip:
                    ...
                    if re.search("addon|changelog|icon|fanart", y):
                        shutil.copyfile(os.path.join(ROOT_DIR, x, y), os.path.join(zipsfolder, y))
